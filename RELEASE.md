...
Push a version tag to kick off a full release build:
...
1. GitHub Actions builds 4 platform artifacts **in parallel**:
   - `mac-arm64` — signed + notarized macOS DMG (Apple Silicon)
   - `mac-x64` — signed + notarized macOS DMG (Intel)
   - `win-x64` — Windows installer
...
3. All artifacts are attached to the release automatically.
