...
> **Current release scope: macOS only.** The workflow file references multi-platform targets, but only macOS artifacts are built and distributed for v0.1.x. Windows and Linux targets are not current release scope. Current releases are unsigned DMG (no Apple credentials yet); users must bypass Gatekeeper manually — see the Gatekeeper waiver instructions below.

Releases are driven by GitHub Actions. Manual steps are only needed for local macOS builds — see the sections below.

### Triggering a Release

Push a version tag to kick off a release build:
...
1. GitHub Actions builds the macOS release artifact:
   - `mac-arm64` — macOS DMG (Apple Silicon)
   - When Apple credentials are available: signed + notarized DMG
   - Without credentials: unsigned DMG requiring manual Gatekeeper bypass
2. A GitHub Release is auto-created with a changelog generated by **git-cliff** from commit history.
3. Artifacts are attached to the release automatically.
...
| `GITHUB_TOKEN` | Release creation — **auto-provided by GitHub, no setup needed** |

### CI on Every Push / PR

On every push or PR targeting `main`, CI runs:
- Go tests (`go test ./...`)
...
## 8. Gatekeeper Bypass (Unsigned DMG Release)

> **Required when distributing an unsigned DMG** — current v0.1.x releases are unsigned. Users on macOS must bypass Gatekeeper before first launch.

When Apple credentials are not available and the release is an unsigned DMG:

1. Download and mount the DMG.
2. Drag `Astraler Skillbox.app` to your Applications folder (or another location).
3. Before launching, remove the quarantine flag:
   ```sh
   xattr -r -d com.apple.quarantine "/Applications/Astraler Skillbox.app"
   ```
4. Launch the app normally.

Include these instructions in the GitHub Release notes until Apple signing/notarization credentials are configured and a credentialed release is produced.

## 9. Current State on This Machine (No Credentials)
