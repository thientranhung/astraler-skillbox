...
Mỗi provider dùng nguồn khác nhau để lấy version (áp dụng cho cả user layer và
project layer trừ khi ghi chú riêng):
...
Cache dir là global (không có per-project cache trong Codex), nên cùng một version
map được dùng cho cả user layer và project layer trong `scanProjectInternal`.
