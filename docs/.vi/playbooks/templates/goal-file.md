...
── TIÊU CHÍ THÀNH CÔNG (TẤT CẢ PHẢI ĐÚNG) ──

1. <Kết quả cụ thể đo được>
2. <Kết quả cụ thể đo được>
3. <Kết quả cụ thể đo được>
4. Deliverable cuối chạy/build không lỗi (tests pass nếu trong scope)
...
── LUẬT VẬN HÀNH — KHÔNG THƯƠNG LƯỢNG ──
...
7. PROGRESS LOG. Ghi completed / in-flight / decisions / blockers trong response cuối phase.
...
10. CHECK SUCCESS BEFORE STOPPING. Đọc lại TIÊU CHÍ THÀNH CÔNG, xác nhận từng dòng đã đạt; nếu không, fix trước khi báo done.

── THANH CHẤT LƯỢNG ──

- Code: clean, typed, theo conventions (`AGENTS.md` § Conventions).
- Architecture: tôn trọng hard rules trong `docs/10-technical-architecture.md` (boundary renderer / main / Go).
- Output: qua được Larry code review + smoke tests.
- Docs: concept mới / RPC mới / migration mới → update doc canonical theo `documentation.md` map.
- Commit: imperative ngắn, kèm trailer `DOC-VERIFIED: <reason>` nếu push range chạm `main`.
