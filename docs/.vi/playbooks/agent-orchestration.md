...
3. **OPERATING RULES** — 10 luật không thương lượng (Plan first, Respect phase gate, Self-verify, Debug yourself, Use every tool, No placeholders, Progress log, Stay on scope, If blocked, Check success before stopping). Adapt từ template gốc: rule #2 thay "WORK AUTONOMOUSLY" bằng "**RESPECT PHASE GATE**" — dừng đúng phase, không chạy gộp spec → code.
4. **QUALITY BAR** — code conventions, architecture hard rules (`docs/10`), qua được Larry review, docs cập nhật, commit trailer `DOC-VERIFIED`.
...
> **Gotcha same-owner (sẽ lặp lại):** GitHub chặn `gh pr review --approve` / `--request-changes` trên PR của *chính account đó* ("Can not approve/request changes on your own pull request"). Reviewer chuyển sang đăng verdict bằng `gh pr comment` — ghi rõ **APPROVE / BLOCK + file:line**; Orchestrator đọc comment làm verdict. Lưu ý `reviewDecision` sẽ **rỗng** dù đã review xong — đừng tưởng "chưa review". Vẫn dùng CI checks (`gh pr checks`) + `mergeStateStatus=CLEAN` làm điều kiện merge.
